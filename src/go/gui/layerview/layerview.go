...
func (lv *LayerView) Render(ctx *nk.Context, img *img.Image) bool {
	...
	if nk.NkGroupBegin(ctx, "LayerTabPane", nk.WindowNoScrollbar) != 0 {
		...
		if nk.NkSelectLabel(ctx, locale.Pgettext("layerview", "Layers"), nk.TextAlignCentered|nk.TextAlignMiddle, b2i(lv.layerFavSelectedIndex == 0)) != b2i(lv.layerFavSelectedIndex == 0) {
			lv.layerFavSelectedIndex = 0
		}
		if img.PFV != nil && len(img.PFV.FaviewRoot.Children) > 0 {
			if nk.NkSelectLabel(ctx, locale.Pgettext("layerview", "Simple View"), nk.TextAlignCentered|nk.TextAlignMiddle, b2i(lv.layerFavSelectedIndex == 1)) != b2i(lv.layerFavSelectedIndex == 1) {
				lv.layerFavSelectedIndex = 1
			}
		}
		if img.PFV != nil {
			if nk.NkSelectLabel(ctx, locale.Pgettext("layerview", "Favorites"), nk.TextAlignCentered|nk.TextAlignMiddle, b2i(lv.layerFavSelectedIndex == 2)) != b2i(lv.layerFavSelectedIndex == 2) {
...
func (lv *LayerView) layerTreeItem(ctx *nk.Context, indent, thumbSize float32, thumb *nk.Image, visible, forceVisible bool, l *composite.Layer) int {
	...
	if state != nk.WidgetRom {
		if nk.NkWidgetIsHovered(ctx) != 0 {
			bg.SetA(32)
			if !forceVisible && nk.NkInputIsMousePressed(ctx.Input(), nk.ButtonLeft) != 0 {
				clicked |= 1
			}
